...
def shard_fp8_moe_weights_to_tpu(
    ...
    on TPU in parallel across experts.
...
def process_fp8_moe_weights_scanned(
    weights: FusedMoEWeights,
    moe_backend: MoEBackend,
    mesh: Mesh,
    activation: str,
    ...
    """Wrapper around process_fp8_moe_weights that uses lax.scan.

    Processes one expert at a time via jax.lax.scan to reduce peak XLA
    memory reservation compared to processing all experts simultaneously.
    """
    w13_weight = weights.w13_weight
    w13_weight_scale = weights.w13_weight_scale
    w2_weight = weights.w2_weight
    ...
    def _process_single_expert(carry, xs):
        w13_e, w13_s_e, w2_e, w2_s_e = xs
        single = FusedMoEWeights(
            w13_weight=w13_e[jnp.newaxis],
            w13_weight_scale=w13_s_e[jnp.newaxis],
            w13_bias=None,
            w2_weight=w2_e[jnp.newaxis],
            w2_weight_scale=w2_s_e[jnp.newaxis],
            w2_bias=None,
        )
        result = process_fp8_moe_weights(
            single,
            moe_backend=moe_backend,
            mesh=mesh,
        ...
        return carry, (
            result.w13_weight[0],
            result.w13_weight_scale[0],
            result.w2_weight[0],
            result.w2_weight_scale[0],
        )

    _, (w13_q, w13_s, w2_q, w2_s) = jax.lax.scan(
        _process_single_expert,
        init=None,
